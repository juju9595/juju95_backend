...
public class Example2 { //class s
    ...
    public static void main(String[] args) {//main s
        ...
        [ static ] *정적인
            1. 프로젝트 실행될 때 메모리에 할당되고 프로그램이 종료될때 메모리 삭제되는 키워드
            2. 사용법 : static 타입 멤버변수 -------> static(정적)변수
            3. 주의할점 : 멤버변수(객체내변수) 공유x new vs static(우선할당) 공유 O
                1) 프로그램내 딱 1개만 존재하는 변수를 선언할때 사용. 서로 다른 객체들이 공유해서 사용한다.
         [ 상수 ] : 변하지 않는 변수
            1. 사용법 : static final 타입 상수명;
            2. 주의할점 : 1)수정불가능 2)프로그램내 딱1개 존재 3)상수명 대문자 사용

         [ static 유무 중요한 이유 ]
            !!!!!!!!: static 으로 선언된 메소드는 같은 클래스내 메소드를 호출할 수 없다.
            // 방법 1: 객체를 생성하여 객체를 통해 static 아닌 메소드 호출한다.

            class Test {
                static void main3(){ }
                void main2(){ }
                static void main(){
                    main2(){}   //불가능: main 함수는 우선할당이 되면서 main2가 모른다. 누구세요?
                    new Test().main2();   //가능 : static 없는 경우 객체변수
                    main3();    //가능: main함수는 우선할당이 되면서 우선할당된 main3가 알고있다.
                }
            }
         */
