...
select mid as 회원아이디 , mname 회원명 from member;		-- as는 생략하고 띄어쓰기로 구분이 가능하다.
...
select *from member where mnumber = 4;			-- member테이블의 mnumber속성의 값이 4이면
...
select *from member where mheight >= 165 and mheight <= 170;-- mheight속성의 값이 값이 165이상 이면서 170이하
select *from member where mheight between 165 and 170;		-- mheight속성의 값이 값이 165이상 이면서 170이하
...
select *from member where mphone1 is null;		-- 속성값이 null 이면 [ㅇ가능]
select *from member where mphone1 is not null;	-- 속성값이 null 이 아니면
# null연산자 : is null , 속성명 is not null
...
select *from member where mname like '_이_';		-- 속성의 값이 두번째 글자가 '이'인 세글자
# 문자패턴 : 속성명 like '문자패턴' , %:문자대응, _:문자대응(_수만큼)

# [5] order by 속성명 asc/desc , 정렬 , acc(오름차순/기본값) desc(내림차순)
# 오름차순 : 1 2 3 4 , a b c , ㄱ ㄴ ㄷ, 25-07-15 25-07-16 25-07-17
# 내림차순 : 4 3 2 1 , c b a , ㄷ ㄴ ㅋ, 25-07-17 25-07-16 25-07-15
