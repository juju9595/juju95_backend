...
create table test(
    ...
    constraint primary key(mno) #식별키 설정 # 마지막 속성명 뒤에, (쉼표) 생략    
);
# [*] DML : 데이터베이스 조작어 
...
# 주의할점 : 테이블내 데이터 사입시 레코드 단위, 한칸한칸에 데이터삽입이 아닌 줄/행/레코드 단위 삽입
# (1) insert into 테이블명( 속성명1, 속성명2) values( 값1, 값2 );
insert into test(mno, mname, mcount) values(1,"유재석", 30); # ;(세미클론) 마침 
# 작성한 속성명에 대응하는 값 삽입한다.
insert into test(mcount, mno, mname) value(40, 2, "강호동");			# ctrl+enter 실행
# (2) auto_increment 제약조건의 속성은 값 생략할경우 자동번호(1씩증가된) 할당 	#PK에서 주로 사용
insert into test( mcount, mname) values (50, "신동엽");
#(3)default 제약조건의 속성은 값 생략할 경우 정의된 기본값이 할당				# 가입일
insert into test(mname) values("서장훈");
#(4) not null 제약조건의 속성은 값이 꼭 삽입해야한다. Error Code : 1364. Field 
...
#(7) 하나의 inset에서 여러개 레코드 삽입할 경우, values(),(),();
insert into test values(6, "박명수", 50) , ( 7,"박명수2", 60 );
select *from test; #확인차 조회

# [2] SELECT : 레코드 조회
# 주의할점 : 조회 결과는 기존 테이블과 상관없다. select 만 result table 존재한다.
...
# 'test' 테이블에서 'mname'속성 값이 '유재석' 인 레코드를 조회
...
set SQL_SAFE_UPDATES = 0;	#safe mode 끄기, 반대로 1기 켜기 
...
*mysqlWorkbench safeMode : pc마다 첫 설치 이후 safeMode 켜져 있음.
