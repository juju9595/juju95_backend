...
# ( :정의시작 ,):정의끝, 필드명: 아무거나 속성명 , int :속성의 타입 
...
create table waiting(
    ...
    count tinyint			#대기인원수  # 최대 100정도	#pricate byte count
);
select * from waiting;
...
create table test4(
	# 필드명 타입 제약조건
	필드명1 tinyint not null, 		-- 지정한 속성에는 null를 저장할 수 없도록 조건 설정
...
# 예3] 회원제 게시판 DB 설계하시오.
drop database if exists boardService0715;		-- 새로 만들 데이터베이스가 존재하면 삭세
create database boardService0715;				-- 데이터베이스 생성
use boardService0715;							-- 데이터베이스 활성화
create table member(							-- [1] 회원테이블
	mno int auto_increment ,					-- 회원번호는 : 정수, 자도번호 , PK사용할 예정
    ...
    constraint primary key(mno)					-- 위에서 선언된 '회원번호' 속성을 PK필드를 설정
);
...
create table board(								-- [2] 게시물 테이블
    ...
    bcontent longtext,							-- 게시물내용 : 4GB대용량(사진/긴글)
    bdate datetime default now(),				-- 게시물작성일 : 날짜/시간, 기본값, now() : 현재날짜/시간 반환함수
    bview int default 0,						-- 게시물조회수 : 정수 , 기본값 , 0
    constraint primary key(bno),				-- 위에서 선언된 '게시물번호' 속성을 *PK필드로 설정
    mno int, 									-- 게시물작성자 : "FK 사용할 예정 --> PK필드 타입과 일치
