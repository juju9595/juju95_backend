...
-- 2. 전체실행 : 현재 파일내 모든 명령어(문장) 실행(위에서아래로) ctrl+shift+enter 혹은 상단메뉴 번개 아이콘 출력 
-- [4] 실행 결과 보는법
-- 1. 실행후 메뉴 하단에 [result grid] 혹은 [output]창 확인하여 명령문 결과 확인한다.

-- [5] 데이터베이스 관련 주요 명령어
# 1. 데이터베이스가 저장된 DB서버 로컬 경로 확인하는 방법
...
# 주의할점 : 실행 후 하단에 [output]창 확인하면Error Code: 1007. Can't create database 'mydb0714'; database exists 이미 존재하는 DB명

# 3. DB서버내 데이터베이스 를 삭제하기
# drop(버린다는뜻), mydb0714(삭제할 db명)
drop database mydb0714;			# 두번 실행하면 'xxxx'; database doesn't exist : 존재하지 않는 DB명

# 4. DB서버내 데이터베이스가 *존재*하면 삭제하기
# if(만약에 뜻), exists(존재하다는뜻)
drop database if exists mydb0714; # 여러번 실행해도 오류가 없다.alter

# 5. DB서버내 데이터베이스 목록 보기
# show(보다뜻), database*s(데이터베이스목록)
show databases;		# [grid result] 창에서 확인

#6. DB서버내 사용할 데이터베이스 활성화 하기 , 여러 db중에서 sql 사용할 db 꼭 선택
# use(사용하다 뜻), 
use mydb0714;

# UI 에서 데이터베이스 확인 : 워크벤치 왼쪽 메뉴 [navigator] ->[새로고치]아이콘 클릭
...
use test0714;						#'test0714' 의 이름이 데이터베이스 (사용) 활성화 하기 
