...
-- JOIN 필요한 이유 : 제품명 과 카테고리명 은 서로 다른 테이블에 존재한다. 즉] 서로 다른 테이블을 하나로 집합 하기alter
-- !! JOIN 조건 : 서로 다른 테이블간의 동일한 속성값이 필요하다. 주로] PK와 FK 속성이 주로 같은 값을 갖는다. 집주소 배송지
select *from product p; 				-- 1. '제품명'이 존재하는 테이블 조회
select *from pcategory pc;				-- 2. '카테고리명'이 존재하는 테이블 조회
select *from product p , pcategory pc;	-- *. 서로 다른 두 테이블을 한번에 조회( 교집합 x )
-- 3. 교집합 ( 두 집합[테이블] 간의 공통, 주로 pk와 fk ) 
...
-- 3개이상의 테이블 조인 : 테이블 A INNER JOIN 테이블B ON 테이블A_PK = 테이블B_FK INNER JOIN 테이블C ON 테이블B_PK = 테이블C_FK
...
-- 카테고리명 : pcategory / 제품명 : product / 교집합 : 카테고리번호pk-fk
...
-- 제품명 : product / 재고 : stock / 교집합 : 제품번호pk-fk
...
select p.제품명 
	...
	where s.재고수량 is null; -- 차집합

-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
-- 카테고리명 : pcategory / 재고수량 : stock	/ 교집합 : 카테고리랑 재고의 교집합이 없다.
-- 카테고리 --> 제품 --> 재고				/ 카테고리-제품 교집합 , 제품-재고 교집합 
select pc.카테고리명 , sum(s.재고수량) 
from pcategory pc 
inner join product p on pc.카테고리번호_pk = p.카테고리번호_fk
inner join stock s on p.제품번호_pk = s.제품번호_fk
group by pc.카테고리명;
...
-- 제품 : product / 재고 : stock / 교집합 : 제품번호pk-fk
