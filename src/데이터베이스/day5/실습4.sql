...
select *from orders;
select avg(price) as 평균단가 from books as b; 

-- [문제 3] orders 테이블에서 '김민준' 고객이 주문한 총 주문 수량(order_qty의 합계)을 조회하세요.
select * from orders;
select * from orders where customer = '김민준';
select sum(order_qty) from orders where customer = '김민준'; #customer(고객명) 그룹전에 조건을 사용 = where vs having
...
-- [문제 6] '김민준'과 '이서연' 고객에 대해, 각 고객별 총 주문 수량(order_qty의 합계)을 조회하세요. 
-- 단, 총 주문 수량이 4권 이상인 고객만 조회하세요.
select *from orders;
select *from orders where customer = '김민준' or customer ='이서연'; #'김민준'과 '이서연' 고객에 대해 ( 그룹전에 가능한 조건) = where
select *from orders where customer in('김민준', '이서연');
#각 고객별 
select customer from orers where customer in('김민준', '이서연') group by customer; 
#총 주문 수량이 4권 이상인 고객만 조회
select customer, sum(order_qty)as 총주문수량 
from orders where customer in('김민준', '이서연') 
group by customer having 총주문수량 > 4;
...
select author, count(*)as 출판도서수 from books group by author having 출판도서수 >= 2; #저자별 레코드수 == 출발한도서수
-- [문제 11] books 테이블의 '컴퓨터' 장르 도서 중 가장 비싼 도서의 가격을 조회하세요.
select price from books where genre = '컴퓨터' order by price desc limit 1;
