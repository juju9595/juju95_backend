...
select bpname from buy group by bpname; -- buy 테이블의 'npname'속성 그룹
-- select bpname , bprice form buy group by bpname; 
-- 오류 : bpname 속성값이 동일하더라도 bprice 다를 수 있기 때문에,
...
# [3] having : 그룹(후) 조건 : 별칭o vs where 그룹(전) 조건 : 별칭x
# 1. 구매수량이 3 초과 하고 회원아이디 별 레코드 조회 
select *from buy where bamount > 3;
-- select bamount as 수량 from buy where 수량 > 3; #실행 불가능
...
-- select mid, sum(bmaount*bprice) as 총구매금액 from buy where 총구매금액 >= 1000 group by mid; #실행 불가능
-- select count(*) from buy where count(*) > 3; #실행 불가능 where 에서는 집계 사용 불가
