...
public class Example1 {
    public static void main(String[] args) {
        ...
        [ 스레드 Thread ]
            ...
            3. 자바에서의 스레드
                1) main메소드 : public static void main(String[] args){} 에는 main 스레드가 내장 됌.
            4. 사용법
                1) 자바 프로그램은 항상 main메소드(Thread)로 부터 실행 흐름 시작된다.
                - public static void main(String[] args){}

         [ 멀티스레드 Muti-Thread ]
            1. 정의 : main스레드 외 새로운 *작업스레드*를 생성하여 동시 작업
            2. 목적 : *병렬 처리* , 동시에 여러 작업을 수행
        ...
        for(int i = 1; i<=5; i++){
            ...
            try {
                Thread.sleep(1000);// Thread.sleep(밀리초) : 현재 스레드를 밀리초 만큼 일시정지 *예외발생
            }catch (Exception e){}
        }//for e

        // [2] 멀티 스레드 예 ) 익명 구현체 : 인터페이스타입을 클레스없이 직접 구현
        // 1) 출력 5번
        for(int i = 1; i <=5; i ++){
        ...
        // 2) 소리 5번 : 익명 구현체 , new 인터페이스타입(){ 추사메소드구현 };
        Runnable runnable1 = new Runnable(){
            // run 추상메소드구현 // 오버라이딩 // 재정의
            @Override
            public void run() {
                for(int i = 1; i <=5; i ++){
                    toolkit.beep();
        ...
        // [3] 멀티 스레드 예2 :  구현체
        //1) 소리 5번
        작업스레드1 작업스레드1 = new 작업스레드1(); //구현제
        Thread thread2 = new Thread(작업스레드1); // 구현체를 Thread class 대입
