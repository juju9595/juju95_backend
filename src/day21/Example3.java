...
// (2) 유저 클래스 [작업스레드2]
    ...
    class Calculator{
        ...
        public synchronized void setMemory(int memory){
            //synchronized : 두 개 이상으 ㅣ스레드가 해당 메소드 사용시 먼저 요청한 스레드가 점령하고 종료될때까지 다른 스레드는 awit(대기) 안된다.
            ...
            // 동기화 목적 : 두개 이상의 스레드가 먼저 메소드를 호출한 순서대로 다른 스레드는 await 상태로 만든다.
...
public class Example3 {
    public static void main(String[] args) {
        ...
        /*
        [synchronized] 동기화
            1. 정의 : 메소드에 lock(락)을 사용하여 메소드를 정령한다.
            2. 목적 : 여러 스레드가 동시에 하나의 스레드를 사용하면 멤버변수(코드) 충돌이 생길 수 있다.
            3. 비교
                동기화                                 비동기화(기본값)
                한번에 하나의 스레드가 처리               여러개의 스레드가 동시 처리
                처리 순서보장 보장 됌                    처리 순서 보장 안됌.
                synchronized                          x
                주문순서대로 음료 제조후 손님에게 준다.     제조가 먼저 완성된 음료를 순서대로 손님에게 준다.
            4. 사용법
        ...
        user2.start(); //user2는 200 넣기 : 200 -> synchronized -> 200
