...
public class UserDao {
    ...
    private  String db_usr = "root";        // DB서버의 계정명     = root
    ...
    public void connect(){
        try{
            ...
            conn = DriverManager.getConnection( db_url, db_usr, db_password);// 2. MYSQL DB서버 연동
        }catch (ClassNotFoundException e){
            System.out.println("[경고] mysql 라이브러리 로드 실패");
        }catch ( SQLException e){System.out.println("[경고] DB 서버와 연동 실패");}
        // 2. MYSQL 드라이버/클래스 불러오기
    }//func e
    ...
    // 왜? 기능은 하나만 있어도 재사용 가능하니깐. 즉] 메소드 위주 , 요리사사람1명, 서빙사람1명
    // -> D(데이터)T(이동)O(객체)에서는 싱글톤 사용하지 않는다.
    // 왜? 데이터(회원정보)1개만 존재하지 않는다. 즉] 멤버변수 위주 , 라면20개

    // (2) 연동 성공 이후에 사용한 DML(테이블 조작) 하기
    ...
    public boolean userInsert2( String uname, int uage){
        ...
        try {
            ...
            // SQL 문법내 첫번째 ?에 uname 변수값 String 타입으로 대입
            ps.setInt(2, uage);
            //SQL 문법내 두번째 ?에 uage 변수값을 int 타입으로 대입한다.
            // 4. SQL 실행하기
            int count = ps.executeUpdate();
            // 5. SQL 결과에 따른 확인/로직/리턴
            if(count >= 1)return true; // 결과가 1이상이면 성공
            return false; // 결과가 1미만이면 실패
        } catch (Exception e) {System.out.println(e);}
        return false; // 예외(catch) 발생하며 실패
    }//func e
        ...
        try {
            ...
            while (rs.next()){//-- 다음 레코드가 존재하지 않을때 까지 반복
                ...
                System.out.printf("번호 : %d, 이름 : %s, 나이 : %s \n",
                        ...
                        rs.getString(2), // -- 두번째 열/속성/컬럼 의 값 반환
                        rs.getInt(3)); // -- 세번째 열/속성/컬럼 의 값 반환
                // (2) 현재 순회/반복 중인 레코드의 열/속성/컬럼 값을 DTO로 구성
                // CSV/데이터베이스 : 테이블 처럼 행과 열로 구성
                // 자바 : 객체지향 구성, 즉] 레코드/행 1개 == 객체1개 , 여러개 행 == 리스트/배열
            }
        } catch (Exception e) {System.out.println(e);}
    }//func e
}//class e
