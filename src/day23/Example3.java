...
public class Example3 {
    public static void main(String[] args) {
        ...
        map.put("신동엽",78);
        map.put("유재석",67); // key(속성명) 중복 불가능하다. (*식별자*)
        map.put("서장훈",100); // value(속성값) 중복이 가능하다.
        // 변수명(상징적인이름), 변수값(상징적인이름에 저장된 실질적인 자료)
        ...
        map.remove("유재석");
        System.out.println(map); //{서장훈=100, 강호동=100, 신동엽=78} -> value 도 함께 삭제 됌.
            // 5) .containsKey(key)
        ...
        for(String key : map.keySet()){//map 객체내 모든 키를 반복한다.
            ...
            System.out.println(map.get(key));   //value * key 활용하여 value 호출 한다.[d]
        }
        // 3) forEach 문
