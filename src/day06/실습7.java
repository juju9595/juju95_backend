...
//        prod1.name = "새우깡";
//        prod1.price = 1500; // 각각 "새우깡", 1500
//        prod2.name = "콜라";
//        prod2.price = 2000; // 각각 "콜라", 2000
...
//
        ...
        MenuItem[] menuList = {m1, m2, m3}; // MenuItem 배열

        for (int i = 0; i < menuList.length; i++) {
            if (menuList[i].isSignature == true) {//if문을 사용하여 isSignature가 true인 메뉴 객체를 찾아
                System.out.println("[대표메뉴] " + menuList[i].name + " : " + menuList[i].price + "원");//"[대표메뉴] [메뉴이름] : [가격]원" 형식으로 출력
            }
        }
...
//        3. 모든 정보가 저장된 객체의 멤버 변수들을 가져와 "--- 프로필 ---", "이름: [이름]", "나이: [나이]", "MBTI: [MBTI]" 형식으로 출력하세요.Scanner scan = new Scanner(System.in);
        ...
        int age = scan.nextInt();//입력받은 값들을 생성된 객체의 각 멤버 변수에 저장하세요.

        System.out.println("MBTI: ");
        String mbti = scan.next();//입력받은 값들을 생성된 객체의 각 멤버 변수에 저장하세요.

        //"--- 프로필 ---", "이름: [이름]", "나이: [나이]", "MBTI: [MBTI]" 형식으로 출력
        System.out.print("--- 프로필 ---");
        System.out.print(", 이름:" + name);
        System.out.print(", MBTI:" + mbti);
